...
export class TutorChain {
    ...
    private buildSystemPrompt(input: TutorInteraction): string {
        ...
        // Add specific instructions based on the topic
        let specificInstructions = '';
        if (input.currentTopic.toLowerCase().includes('security')) {
            specificInstructions = 'Include security considerations, risks, and best practices. Mention sanitization, escaping, and security implications.';
        } else if (input.currentTopic.toLowerCase().includes('performance')) {
            specificInstructions = 'Include performance considerations, complexity analysis (Big O notation), and optimization techniques like memoization.';
        } else if (input.currentTopic.toLowerCase().includes('browser') || input.currentTopic.toLowerCase().includes('web')) {
            specificInstructions = 'Address browser compatibility, polyfills, and cross-platform considerations.';
        }
        ...
        return `You are an AI tutor helping a ${input.skillLevel} level student with ${input.currentTopic}.
            ...
            Always mention common mistakes, pitfalls, and error handling strategies.
...
class ExampleClass {
    ...
    private extractResources(content: string): string[] {
        ...
        // Add default resources if none found
        if (resources.length === 0) {
            resources.push('https://reactjs.org/docs/getting-started.html');
            resources.push('https://developer.mozilla.org/en-US/docs/Web/JavaScript');
        }
