...
function _convertToValidToolId(model, tool_call_id) {
    if (model.startsWith("mistralai"))
        return (0, ibm_js_1._convertToolCallIdToMistralCompatible)(tool_call_id);
    else
        return tool_call_id;
}
...
class ChatWatsonx extends chat_models_1.BaseChatModel {
    static lc_name() {
        return "ChatWatsonx";
    }
    get lc_secrets() {
    ...
    withStructuredOutput(outputSchema, config) {
        ...
        if (method === "jsonMode") {
            ...
            if ((0, types_1.isZodSchema)(schema)) {
                ...
                outputParser = new ibm_js_1.WatsonxToolsOutputParser({
                    returnSingle: true,
